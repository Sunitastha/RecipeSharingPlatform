...
const getAll = async (req, res) => {
    try {
        const data = await Recipe.find({}).populate('createdBy', 'fullName username email gender');
        res.status(200).send({
            status: 200,
...
const getById = async (req, res) => {
    try {
        const data = await Recipe.findById(req.params.id).populate('createdBy', 'fullName username email gender');
        if (data) {
            res.status(200).send({
                status: 200,
                message: 'Data retrieved',
...
const getRecipesByUserId = async (req, res) => {
    try {
        ...
        const recipes = await Recipe.find({ createdBy: userId }).populate('createdBy', 'fullName username email gender');
